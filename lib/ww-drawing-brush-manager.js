...
var WwDrawingBrushManager = (function () {
    ...
    _createClass(WwDrawingBrushManager, [{
        ...
        value: function init() {
            ...
            var mode = arguments[1] === undefined ? 'canvas' : arguments[1];
            var brushes_obj = arguments[2] === undefined ? null : arguments[2];
            ...
            if (!this.brushesObject) {
                ...
                (function () {
                    var path = _this.brushesObject.path ? _this.brushesObject.path : './images/brushes/';
                    var list = _this.brushesObject.list;

                    if (!list) {
                        console.log('WwDrawingBrushManager: brushes list must be a valid array of brush names!');
                    } else {
                        list.forEach(function (brush_filename) {
                            var parts = brush_filename.split('.');
                            var id = parts[0];
                            _this.brushIdArray.push(id);
                            _this.brushURLs[id] = path + brush_filename;
                            console.log(_this.brushURLs[id]);
                        });
                    }
                })();
            }
            ...
            */
            ...
            for (var _key in this.brushURLs) {
                var _brush = new _wwBrush2['default'](this.mode);
