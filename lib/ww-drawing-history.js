...
var WwDrawingHistory = (function () {
    function WwDrawingHistory() {
    ...
    _createClass(WwDrawingHistory, [{
        key: 'toString',
        value: function toString() {
            var result = 'Start: ' + this.startTime + ', duration: ' + this.duration + ', end: ' + (this.startTime + this.duration) + '\n';
        ...
        key: 'concatAllCommands',
        value: function concatAllCommands() {
            var _this = this;

            var layer = arguments[0] === undefined ? null : arguments[0];

            var unit = new _wwDrawingHistoryUnit2['default']();

            this.units.forEach(function (temp_unit) {
                if (temp_unit) {
                    temp_unit.commands.forEach(function (temp_command) {

                        if (_this.startTime == 0) {
                            _this.startTime = temp_command.executionTime;
                        } else {
                            _this.startTime = Math.min(_this.startTime, temp_command.executionTime);
                        }
                        _this.duration = Math.max(_this.duration, temp_command.executionTime);
                        if (layer) {
