...
var WwDrawingHistoryDataTranslator = (function () {
    function WwDrawingHistoryDataTranslator() {
        _classCallCheck(this, WwDrawingHistoryDataTranslator);

        this.previousCommandData = new _wwDrawingHistoryBrushCommand2['default']('', null, 1, 0, 0, '', -1, 0, -1, '');
        this.drawingScale = 1.0;
    }

    _createClass(WwDrawingHistoryDataTranslator, [{
        key: 'generateCommandWithDataObject',
        value: function generateCommandWithDataObject(data_object) {
            var command = new _wwDrawingHistoryBrushCommand2['default']('');
            ...
            var _x = this.previousCommandData.updateX(data_object.x);
            var _y = this.previousCommandData.updateY(data_object.y);

            _x = Math.floor(_x * 10) / 10;
            _y = Math.floor(_y * 10) / 10;

            var _scale = _wwDeviceInfo2['default'].instance.assetScaleFactor / this.drawingScale; //  1.0;  // 0.5 for iPad 2
            command.location = new _point2['default'](_x * _scale, _y * _scale);

            return command;
        }
    }, {
        key: 'generateDataObjectWithCommand',
        value: function generateDataObjectWithCommand(command) {
            var data_object = {};
    ...
    return WwDrawingHistoryDataTranslator;
})();

exports['default'] = WwDrawingHistoryDataTranslator;
...
//# sourceMappingURL=ww-drawing-history-data-translator.js.map